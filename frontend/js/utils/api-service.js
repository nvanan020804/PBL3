...
const fetchGet = async (endpoint) => {
  try {
    ...
    if (!connected) {
      throw new Error(
        "Không thể kết nối đến server. Kiểm tra kết nối mạng hoặc server."
      );
    }
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Lỗi từ server: ${response.status}${errorText ? " - " + errorText : ""}`
      );
    }
...
const fetchPost = async (endpoint, data) => {
  try {
    ...
    console.log(
      `POST request to ${endpoint} - Data size: ${dataSizeMB.toFixed(2)}MB`
    );

    // Cảnh báo hoặc từ chối nếu dữ liệu quá lớn
    if (dataSizeMB > 5) {
      console.error(
        `Data size too large (${dataSizeMB.toFixed(
          2
        )}MB). Consider reducing image size.`
      );
      throw new Error(
        `Dữ liệu quá lớn (${dataSizeMB.toFixed(
          2
        )}MB). Vui lòng giảm kích thước ảnh.`
      );
    }
...
const fetchPut = async (endpoint, data) => {
  try {
    ...
    // Log kích thước để debug
    console.log(
      `PUT request to ${endpoint} - Data size: ${dataSizeMB.toFixed(2)}MB`
    );
...
const HoaDonAPI = {
  ...
  getRevenueByDay: (startDate, endDate) =>
    fetchGet(`hoadon/revenue/day?startDate=${startDate}&endDate=${endDate}`),

  // Lấy doanh thu theo tháng
  getRevenueByMonth: (startMonth, endMonth) =>
    fetchGet(
      `hoadon/revenue/month?startMonth=${startMonth}&endMonth=${endMonth}`
    ),

  // Lấy doanh thu theo năm
  getRevenueByYear: (startYear, endYear) =>
    fetchGet(`hoadon/revenue/year?startYear=${startYear}&endYear=${endYear}`),

  // Lấy tất cả hóa đơn đã hoàn thành trong khoảng thời gian
  getCompletedInvoices: (startDate, endDate) =>
    fetchGet(`hoadon/completed?startDate=${startDate}&endDate=${endDate}`),
};
...
const HoaDonChiTietAPI = {
  ...
  getHoaDonChiTietByHoaDon: (idHoaDon) =>
    fetchGet(`hoadonchitiet/hoadon/${idHoaDon}`),

  // Tạo chi tiết hóa đơn mới
  createHoaDonChiTiet: (chiTietData) => fetchPost("hoadonchitiet", chiTietData),

  // Cập nhật chi tiết hóa đơn
  updateHoaDonChiTiet: (id, chiTietData) =>
    fetchPut(`hoadonchitiet/${id}`, chiTietData),
...
const DangKyAPI = {
  ...
  getDangKyByKhachHang: (idKhachHang) =>
    fetchGet(`dangky/khachhang/${idKhachHang}`),
  ...
  updateDangKyStatus: (id, statusData) =>
    fetchPut(`dangky/${id}/update-status`, statusData),
};
...
const GoiDichVuAPI = {
  ...
  updateGoiDichVu: (id, goiDichVuData) =>
    fetchPut(`goidichvu/${id}`, goiDichVuData),
...
const KhachHangAPI = {
  ...
  updateKhachHang: (id, khachHangData) =>
    fetchPut(`khachhang/${id}`, khachHangData),
...
const AccountAPI = {
  ...
  registerCustomer: (registrationData) =>
    fetchPost("accounts/register/customer", registrationData),

  // Đăng ký tài khoản nhân viên
  registerStaff: (registrationData) =>
    fetchPost("accounts/register/staff", registrationData),
  ...
  updateAccount: (username, accountData) =>
    fetchPut(`accounts/${username}`, accountData),
