...
function setupEventListeners() {
  ...
  // Đã bỏ chức năng tìm kiếm khách hàng
...
async function loadCustomerNames() {
  try {
    ...
    const customerIds = invoices
      .map((invoice) => {
        if (invoice.khachHang && invoice.khachHang.idKhachHang) {
          ...
          invoice.dangKy &&
          invoice.dangKy.khachHang &&
          invoice.dangKy.khachHang.idKhachHang
        ) {
    ...
    const customerData = await Promise.all(
      customerIds.map((id) => HoaDonAPI.getKhachHangById(id))
    );
...
function displayInvoices() {
  ...
  paginatedInvoices.forEach((invoice) => {
    ...
    row.innerHTML = `
            ...
            <td><span class="status-badge ${statusClass}">${formatStatus(
      invoice.trangThai
    )}</span></td>
            <td><span class="status-badge ${paymentStatusClass}">${formatPaymentStatus(
      invoice.trangThaiThanhToan
    )}</span></td>
...
async function viewInvoiceDetails(invoiceId) {
  try {
    ...
    // Hiển thị thông tin hóa đơn        document.getElementById('viewIdHoaDon').textContent = invoice.idHoaDon;

    // Format thời gian
    document.getElementById("viewCreationTime").textContent = formatDate(
      invoice.thoiGianTao
    );
    ...
    const registrationInfoSection = document.getElementById(
    ...
    if (registrationDetails && registrationDetails.goiDichVu) {
      ...
      document.getElementById("viewTenGoi").textContent =
        registrationDetails.goiDichVu.tenGoi || "N/A";
      document.getElementById("viewNgayBatDau").textContent =
        formatDate(registrationDetails.ngayBatDau) || "N/A";

      // Tính ngày kết thúc từ ngày bắt đầu và thời hạn gói
      if (
        registrationDetails.ngayBatDau &&
        registrationDetails.goiDichVu.thoiHan
      ) {
        ...
        document.getElementById("viewNgayKetThuc").textContent =
          formatDate(ngayKetThuc) || "N/A";
      } else {
        document.getElementById("viewNgayKetThuc").textContent = "N/A";
      }

      document.getElementById("viewTrangThaiDangKy").textContent =
        registrationDetails.trangThai || "N/A";
    } else {
    ...
    document.getElementById("viewPaymentStatus").textContent =
      formatPaymentStatus(invoice.trangThaiThanhToan);

    // Format tiền tệ
    document.getElementById("viewTongTien").textContent = formatCurrency(
      invoice.tongTien
    );
    document.getElementById("viewGiamGia").textContent = formatCurrency(
      invoice.giamGia
    );
    document.getElementById("viewThanhToan").textContent = formatCurrency(
      invoice.thanhToan
    );
    document.getElementById("viewPhuongThuc").textContent = formatPaymentMethod(
      invoice.phuongThuc
    );
    ...
    if (registrationDetails && registrationDetails.goiDichVu) {
      ...
      row.innerHTML = `<td colspan="4" class="text-center">
                ...
                <br><small>Thành tiền: ${formatCurrency(
                  invoice.tongTien
                )}</small>
            </td>`;
      detailsTableBody.appendChild(row);
    ...
    if (invoice.trangThai === "Hoàn thành") {
      ...
      infoText.innerHTML =
        '<i class="fas fa-info-circle"></i> Hóa đơn đã hoàn thành, không thể thay đổi.';
    ...
    if (invoice.trangThai !== "Hoàn thành" && invoice.trangThai !== "Hủy") {
      ...
      if (invoice.trangThaiThanhToan === "Đã thanh toán") {
        ...
        paymentRequiredInfo.innerHTML =
          '<i class="fas fa-exclamation-triangle"></i> Cần thanh toán trước khi hoàn thành';
      ...
      if (invoice.trangThaiThanhToan !== "Đã thanh toán") {
        ...
        markUnpaidBtn.innerHTML =
          '<i class="fas fa-money-bill"></i> Đánh dấu chưa thanh toán';
...
function updateItemPrice() {
  ...
  if (selectedProductId) {
    ...
    const selectedProduct = products.find(
      (p) => p.idSanPham == selectedProductId
    );
...
function addItemToInvoice() {
  try {
    ...
    const soLuong = parseInt(document.getElementById("itemSoLuong").value) || 0;
    const thanhTien =
      parseFloat(document.getElementById("itemThanhTien").value) || 0;
...
async function loadCustomerDetailsForInvoices(invoices) {
  try {
    ...
    invoices.forEach((invoice) => {
      if (
        invoice.khachHang &&
        typeof invoice.khachHang === "object" &&
        invoice.khachHang.idKhachHang
      ) {
    ...
    for (const customerId of customerIds) {
      try {
        const response = await fetch(
          `http://localhost:8080/api/khachhang/${customerId}`
        );
        if (response.ok) {
          ...
          invoices.forEach((invoice) => {
            if (
              invoice.khachHang &&
              invoice.khachHang.idKhachHang === customerId
            ) {
              customerNamesMap[invoice.idHoaDon] = customer.tenKhachHang;
            }
          });
        ...
        console.error(
          `Lỗi khi tải thông tin khách hàng ID ${customerId}:`,
          error
        );
      }
    }
...
async function loadCustomerDetailsForInvoice(invoice) {
  try {
    ...
    if (customerNamesMap[invoice.idHoaDon]) {
      console.log(
        `Thông tin khách hàng cho hóa đơn ${invoice.idHoaDon} đã có sẵn`
      );
      return; // Đã có thông tin khách hàng
    }

    console.log(
      `Đang tải thông tin khách hàng cho hóa đơn ${invoice.idHoaDon}`
    );
    ...
    if (!customerId) {
      console.log(
        `Không tìm thấy ID khách hàng cho hóa đơn ${invoice.idHoaDon}`
      );
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (customer && customer.tenKhachHang) {
          console.log(
            `Đã tìm thấy thông tin khách hàng: ${customer.tenKhachHang}`
          );
          customerNamesMap[invoice.idHoaDon] = customer.tenKhachHang;
        } else {
        ...
        console.log(
          `API trả về lỗi khi tìm khách hàng ID ${customerId}. Status: ${response.status}`
        );
        customerNamesMap[invoice.idHoaDon] = "Khách hàng #" + customerId;
      }
    } catch (error) {
      console.error(
        `Lỗi khi tải thông tin khách hàng cho hóa đơn ${invoice.idHoaDon}:`,
        error
      );
...
async function completeInvoice(invoiceId) {
  try {
    ...
    if (invoice.trangThaiThanhToan !== "Đã thanh toán") {
      showError(
        "Không thể hoàn thành hóa đơn chưa thanh toán. Vui lòng thanh toán hóa đơn trước."
      );
      return;
    }
    ...
    try {
      ...
      if (invoice.idDangKy) {
        try {
          ...
          showError(
            "Hoàn thành hóa đơn thành công nhưng không thể cập nhật trạng thái đăng ký."
          );
        }
      }
      ...
      if (invoice.dangKy && invoice.dangKy.idKhachHang) {
        try {
          ...
          console.log(
            'Đã cập nhật trạng thái khách hàng thành "Đang hoạt động"'
          );
        } catch (custError) {
          console.error("Lỗi khi cập nhật trạng thái khách hàng:", custError);
          showError(
            "Hoàn thành hóa đơn thành công nhưng không thể cập nhật trạng thái khách hàng."
          );
        }
      }
      ...
      // Thêm thông báo cho người dùng về việc làm mới trang sau khi hủy
      showInfo(
        "Đã hủy hóa đơn và đăng ký. Bạn có thể làm mới trang để cập nhật trạng thái mới nhất."
      );
    } catch (error) {
      throw error;
    }
...
async function cancelInvoice(invoiceId) {
  try {
    ...
    try {
      // Thử sử dụng API huyHoaDon
      updatedInvoice = await HoaDonAPI.huyHoaDon(invoiceId);
    } catch (apiError) {
      console.error("Lỗi khi gọi API huyHoaDon:", apiError);

      // Nếu API hủy không tồn tại, sử dụng updateHoaDon làm giải pháp thay thế
      console.log("Sử dụng phương thức updateHoaDon để hủy hóa đơn");

      // Cập nhật trạng thái thành "Hủy"
      updatedInvoice = await HoaDonAPI.updateHoaDon(invoiceId, {
        ...invoice,
        trangThai: "Hủy",
      });
    } // Cập nhật trạng thái đăng ký thành "Đã hủy" thay vì xóa
    if (invoice.idDangKy) {
      try {
        // Sử dụng phương thức update-status để đánh dấu là "Đã hủy"
        await DangKyAPI.updateDangKyStatus(invoice.idDangKy, {
          trangThai: "Đã hủy",
        });
        console.log(
          'Đã cập nhật trạng thái đăng ký thành "Đã hủy". Khách hàng có thể đăng ký lại.'
        );
        showSuccess(
          'Đã cập nhật trạng thái đăng ký thành "Đã hủy". Khách hàng có thể đăng ký gói mới ngay lập tức.'
        );
      } catch (regError) {
        console.error("Lỗi khi cập nhật trạng thái đăng ký:", regError);
        ...
        try {
          await fetch(
            `http://localhost:8080/api/dangky/${invoice.idDangKy}/cancel`,
            {
              method: "PUT",
          ...
          console.log(
            'Đã cập nhật trạng thái đăng ký thành "Đã hủy" qua endpoint /cancel'
          );
          showSuccess(
            "Đã cập nhật trạng thái đăng ký. Khách hàng có thể đăng ký gói mới ngay lập tức."
          );
        } catch (cancelError) {
          console.error("Lỗi khi hủy đăng ký gói dịch vụ:", cancelError);
          showError(
            "Hủy hóa đơn thành công nhưng không thể cập nhật trạng thái đăng ký gói dịch vụ."
          );
        }
      }
    }
    ...
    if (viewStatusElement) {
      viewStatusElement.textContent = "Hủy";
      viewStatusElement.className = "status-badge status-huy";

      // Cập nhật các nút thao tác trong modal để phản ánh trạng thái hủy
      updatePaymentButtons(
        invoiceId,
        updatedInvoice.trangThaiThanhToan === "Đã thanh toán"
      );
    }

    // Đóng modal và mở lại để cập nhật giao diện người dùng
    const viewModal = bootstrap.Modal.getInstance(
      document.getElementById("viewInvoiceModal")
    );
    if (viewModal) {
...
function displayInvoiceDetails() {
  ...
  tempInvoiceDetails.forEach((detail, index) => {
    ...
    row.innerHTML = `
            <td>${
              detail.sanPham ? detail.sanPham.tenSanPham : "Không xác định"
            }</td>
  ...
  if (index >= 0 && index < tempInvoiceDetails.length) {
    ...
    if (detail.sanPham && products) {
      ...
      const productInList = products.find(
        (p) => p.idSanPham === detail.sanPham.idSanPham
      );
      if (productInList) {
        // Cộng lại số lượng sản phẩm vào kho
        productInList.soLuong += detail.soLuong;
        console.log(
          `Đã hoàn trả ${detail.soLuong} sản phẩm "${detail.sanPham.tenSanPham}" vào kho.`
        );
      }
    }
...
async function saveInvoice() {
  try {
    ...
    if (editMode && currentInvoiceId) {
      ...
      savedInvoice = await HoaDonAPI.updateHoaDon(
        currentInvoiceId,
        invoiceData
      );
...
function getInvoiceFormData() {
  try {
    ...
    const giamGia = parseFloat(document.getElementById("giamGia").value) || 0;
    const thanhToan =
      parseFloat(document.getElementById("thanhToan").value) || 0;
    ...
    const trangThaiThanhToan =
      document.getElementById("trangThaiThanhToan").value;
...
// Show info message with better styling
function showInfo(message) {
  const alertElement = document.getElementById("alertMessage");
  alertElement.className = "alert alert-info show";
  alertElement.textContent = message;
  alertElement.classList.remove("d-none");
...
function updateRegistrationDropdown() {
  ...
  if (registrations && registrations.length > 0) {
    const activeRegistrations = registrations.filter(
      (reg) => reg.trangThai === "Đang hoạt động"
    );
    ...
    activeRegistrations.forEach((registration) => {
      ...
      const customerName = registration.khachHang
        ? `${registration.khachHang.ho} ${registration.khachHang.ten}`
        : "Không rõ";
      const packageName = registration.goiDichVu
        ? registration.goiDichVu.tenGoi
        : "Không rõ";
...
function updateInvoiceAmountFromRegistration() {
  ...
  const selectedOption =
    registrationSelect.options[registrationSelect.selectedIndex];
  ...
  if (selectedOption && selectedOption.dataset.price) {
    ...
    if (!isNaN(price)) {
      ...
      const discountAmount =
        parseFloat(document.getElementById("giamGia").value) || 0;
...
function filterAndSearchInvoices() {
  ...
  const paymentStatusValue = document.getElementById(
    "paymentStatusFilter"
  ).value;
  ...
  if (searchValue) {
    filteredInvoices = filteredInvoices.filter((invoice) => {
      ...
      if (invoice.khachHang) {
        customerName =
          `${invoice.khachHang.ho} ${invoice.khachHang.ten}`.toLowerCase();
      } else if (invoice.dangKy && invoice.dangKy.khachHang) {
        customerName =
          `${invoice.dangKy.khachHang.ho} ${invoice.dangKy.khachHang.ten}`.toLowerCase();
      }

      return (
        invoice.idHoaDon.toString().includes(searchValue) ||
  ...
  if (statusValue !== "all") {
    filteredInvoices = filteredInvoices.filter((invoice) => {
      ...
      console.log(
        `Debug - So sánh: ${statusValue} với [${
          invoice.trangThai
        }] => ${formatStatus(invoice.trangThai)
          .toLowerCase()
          .replace(/\s+/g, "")}`
      );
  ...
  if (paymentStatusValue !== "all") {
    filteredInvoices = filteredInvoices.filter((invoice) => {
      ...
      console.log(
        `Debug Payment - So sánh: ${paymentStatusValue} với [${
          invoice.trangThaiThanhToan
        }] => ${formatPaymentStatus(invoice.trangThaiThanhToan)
          .toLowerCase()
          .replace(/\s+/g, "")}`
      ...
      if (
        paymentStatusValue === "dathanhtoan" &&
      ...
      if (
        paymentStatusValue === "chuathanhtoan" &&
        (invoice.trangThaiThanhToan === "Chưa thanh toán" ||
          invoice.trangThaiThanhToan === null ||
          invoice.trangThaiThanhToan === "")
      ) {
        return true;
      }
...
function debugInvoiceStatus() {
  ...
  invoices.forEach((invoice) => {
    ...
    if (
      invoice.trangThaiThanhToan &&
      !uniquePaymentStatuses.includes(invoice.trangThaiThanhToan)
    ) {
  ...
  uniquePaymentStatuses.forEach((status) => {
    const formatted = formatPaymentStatus(status)
      .toLowerCase()
      .replace(/\s+/g, "");
    console.log(
      `Trạng thái thanh toán [${status}] sẽ khớp với filter [${formatted}]`
    );
  });
...
async function searchCustomers(searchTerm) {
  try {
    ...
    const apiUrl = `http://localhost:8080/api/khachhang/search?q=${encodeURIComponent(
      searchTerm
    )}`;
...
function initCustomerSearch() {
  ...
  searchInput.addEventListener("keyup", async function () {
    ...
    try {
      ...
      console.error("Lỗi khi tìm kiếm khách hàng:", error);
      searchResults.innerHTML =
        '<div class="dropdown-item text-danger">Có lỗi xảy ra khi tìm kiếm</div>';
    }
  });
...
function selectCustomer(registration) {
  ...
  customerDetails.innerHTML = `
        <dl class="row mb-0">
            ...
            <dd class="col-sm-8">${
              registration.goiDichVu?.tenGoi || "Không có"
            }</dd>
            
            <dt class="col-sm-4">Thời hạn:</dt>
            <dd class="col-sm-8">${
              formatDate(registration.thoiHan) || "Không có"
            }</dd>
        </dl>
    `;
...
async function markAsPaid(invoiceId) {
  try {
    ...
    // Kiểm tra nếu hóa đơn đã bị hủy thì không cho phép thanh toán
    if (invoice && invoice.trangThai === "Hủy") {
      showError("Không thể thanh toán cho hóa đơn đã hủy.");
      return;
    }
    ...
    if (
      ...
      try {
        ...
        showError(
          "Đã thanh toán nhưng không thể kích hoạt gói đăng ký tự động."
        );
      }
    } else {
...
function updatePaymentButtons(invoiceId, isPaid) {
  ...
  const existingButtons = actionsContainer.querySelectorAll(
    ".payment-status-btn"
  );
  ...
  const invoice = invoices.find((inv) => inv.idHoaDon == invoiceId);
  const isInvoiceCancelled = invoice && invoice.trangThai === "Hủy";

  // Nếu hóa đơn đã bị hủy, không hiển thị các nút thanh toán
  if (isInvoiceCancelled) {
    const cancelInfo = document.createElement("span");
    cancelInfo.className = "text-danger me-2";
    cancelInfo.innerHTML =
      '<i class="fas fa-ban"></i> Hóa đơn đã bị hủy, không thể thanh toán';
    actionsContainer.appendChild(cancelInfo);
    return;
  }
  ...
  try {
    ...
    if (invoice.khachHang && invoice.khachHang.idKhachHang) {
      document.querySelector(
        '#selectedCustomerInfo input[type="hidden"]'
      ).value = invoice.khachHang.idKhachHang;
      document.getElementById(
        "customerNameDisplay"
      ).textContent = `${invoice.khachHang.ho} ${invoice.khachHang.ten}`;
      document.getElementById("customerPhoneDisplay").textContent =
        invoice.khachHang.sdt || "Chưa có";
      document.getElementById("customerEmailDisplay").textContent =
        invoice.khachHang.email || "Chưa có";
    } else {
      document.querySelector(
        '#selectedCustomerInfo input[type="hidden"]'
      ).value = "";
